...
with mid:
    ...
    # --- 3. ABOUT OER ---
    # Updated: Science icon
    sub_section_header("The Science of Water Splitting", icon_class="fa-solid fa-atom")
    main_content_text("""
    ...
    # --- 4. DICTIONARY (Expander) ---
    # Updated: List/Bookmark icon
# --- 4. DICTIONARY (Expander) ---
    with st.expander("Terminology Dictionary", expanded=False):
        # Using markdown to render the Font Awesome icon at the top of the list
        st.markdown("""
    ...
    # --- 6. DATA & ANALYSIS ---
    # Updated: Analysis/Chart icon
    sub_section_header("Expected Outputs & Insights", icon_class="fa-solid fa-magnifying-glass-chart")
    
    col_img1, col_img2, col_img3 = st.columns(3)
    with col_img1:
        st.caption("Gibbs Energy Profile")
        st.markdown("<i class='fa-solid fa-arrow-trend-up' style='color:#007BFF;'></i> *Step-by-step energy levels*", unsafe_allow_html=True)
    with col_img2:
...
# --- 7. EXPERIMENT VS COMPUTATION ---
    with st.expander("Experimental Comparison", expanded=False):
        # Rendering the Font Awesome Microscope icon inside the content
        st.markdown("""
            <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 15px;">
                <i class="fa-solid fa-microscope" style="color: #007BFF; font-size: 1.1rem;"></i>
                <span style="font-weight: 700; color: #1a1c1e;">Experimental vs. Computational Insights</span>
            </div>
        """, unsafe_allow_html=True)
    ...
    # --- 9. NAVIGATION ---
    # Updated: Modern text arrow
    if st.button("Proceed to Pre-requisite →", use_container_width=True):
